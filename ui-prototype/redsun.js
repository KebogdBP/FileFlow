...
function updateFile(file) {
  ...
  uploaded.querySelector('.file-art').textContent = (file.name.split('.').pop() || 'FILE')
    .slice(0, 4)
    .toUpperCase();
}
...
processButton.addEventListener('click', () => {
  ...
  processButton.querySelector('span').textContent = 'Processing locally…';
  setTimeout(() => {
    processButton.classList.remove('loading');
    processButton.querySelector('span').textContent = 'Process file';
